...
namespace EmployeeManagement.Infrastructure.Services.StorageServices
{
    public class LocalStorageService : IStorageService
    {
        ...
        public async Task<string> SaveAsync(string type, byte[] file, string fileName = null)
        {
            ...
            var saveName = Guid.NewGuid().ToString("N") + Path.GetExtension(fileName).ToLower();
        ...
        public Task<(string saveName, string displayName)> SaveAsync(string type, IFormFile file) => SaveAsync(file, _storageLocation.GetPath(type));

        public Task DeleteAsync(string type, string fileName) => Task.Run(() => Delete(_storageLocation.GetPath(type), fileName));

        public Task<bool> DeleteIfExistsAsync(string type, string fileName) => Task.Run(() => DeleteIfExists(_storageLocation.GetPath(type), fileName));
